...
public class PongGeneratorBlock extends GeneratorBlock{
    public PongGeneratorBlock() {
        super("pong_generator");
    }
    @Nullable
    @Override
    public TileEntity createTileEntity(BlockState state, IBlockReader world) {
        return TileInit.PONG_GENERATOR.get().create();
    }
    @Override
    public ActionResultType onBlockActivated(BlockState state, World world, BlockPos blockPos, PlayerEntity player, Hand hand, BlockRayTraceResult hit) {
        if(!world.isRemote) {
            ...
            if(tile instanceof PongGeneratorTileEntity) {
                NetworkHooks.openGui((ServerPlayerEntity) player, (PongGeneratorTileEntity)tile, blockPos);
