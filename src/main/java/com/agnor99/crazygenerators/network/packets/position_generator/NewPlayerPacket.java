...
public class NewPlayerPacket implements Packet {
    ...
    public NewPlayerPacket(PacketBuffer buf) {
        pos = buf.readBlockPos();
        type = DimensionType.getById(buf.readInt());
    }

    public NewPlayerPacket(DimensionType dimension, BlockPos pos) {
        this.pos = pos;
        this.type = dimension;
    }
    ...
    public void doWork(Supplier<NetworkEvent.Context> context) {
        ...
        if(te instanceof PositionGeneratorTileEntity) {
            ...
            pgte.flag.setPlayer(player);
        }
        NetworkUtil.INSTANCE.sendTo(new NewPlayerPacketResponse(player.getName().getFormattedText()), player.connection.getNetworkManager(), NetworkDirection.PLAY_TO_CLIENT);
